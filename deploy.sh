#!/bin/bash
# Build the working tree and deploy it over the installed drapto.
...
print_step "Locating the installed drapto"
if TARGET=$(command -v drapto 2>/dev/null); then
    print_success "$TARGET"
else
    TARGET="$(go env GOPATH)/bin/drapto"
    if [ -x "$TARGET" ]; then
        print_success "$TARGET"
    else
...
go build -trimpath -o "$BUILD" ./cmd/drapto
