...
struct TerminalState {
    ...
    /// Whether we've printed the first encoder message
    first_encoder_message: bool,
...
pub fn print_progress_indicator(message: &str) {
    print_item(OutputLevel::Progress, None, &format!("Progress: {}", message), false);
}
    ...
    if std::io::stderr().is_terminal() {
        ...
        if let Some(pb) = state.current_progress.as_ref() {
            ...
            if let (Some(speed_val), Some(fps_val)) = (speed, fps) {
                if term_width >= 100 {
                    let msg = format!("Encoding - Speed: {:.2}x, Avg FPS: {:.2}", speed_val, fps_val);
                    pb.set_message(msg);
                }
            }
...
impl drapto_core::progress_reporting::ProgressReporter for CliProgressReporter {
    ...
    fn encoder_message(&self, message: &str, is_sample: bool) {
        ...
        let mut state = TERMINAL_STATE.lock().unwrap();
        if !state.first_encoder_message {
            state.first_encoder_message = true;
            debug!("");
        }
        debug!("{}", message);
    }
    ...
    fn encode_start(&self, input_path: &std::path::Path, output_path: &std::path::Path) {
        ...
        if !state.encoding_section_shown {
            ...
            drop(state); // Release lock before printing
    ...
    fn progress_bar(
        ...
        print_progress_bar(percent, elapsed_secs, total_secs, speed, fps, eta);
    }
...
fn format_ffmpeg_simple(args: &[String]) -> String {
    ...
    for arg in args.iter().skip(1) {
        ...
        let needs_new_line = matches!(
            arg.as_str(),
            "-i" | "-hwaccel" | "-vf" | "-af" | "-filter_complex" | 
            "-c:v" | "-c:a" | "-preset" | "-crf" | "-svtav1-params" |
            "-map" | "-y"
        );
