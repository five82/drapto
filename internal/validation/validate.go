...
func validateBitDepth(outputPath string) (bool, *uint8, string) {
	...
	if err == nil {
		...
		if hdr.BitDepth != nil {
			is10Bit := *hdr.BitDepth >= requiredBitDepth
	...
	if props.HDRInfo.BitDepth != nil {
		is10Bit := *props.HDRInfo.BitDepth >= requiredBitDepth
...
func ValidateWithAnalyzer(analyzer MediaAnalyzer, outputPath string, opts Options) (*Result, error) {
	...
	if err != nil {
		...
		result.IsAudioOpus, result.IsAudioTrackCountCorrect, result.AudioCodecs, result.AudioMessage = validateAudioFromAnalyzer(
			audioStreams, opts.ExpectedAudioTracks,
		)
	}
...
// validateAudioFromAnalyzer checks audio codec and track count using analyzer types.
func validateAudioFromAnalyzer(streams []AnalyzerAudioStream, expectedTracks *int) (bool, bool, []string, string) {
	isOpus := true
	var codecs []string
