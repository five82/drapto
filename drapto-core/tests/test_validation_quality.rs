...
fn test_hdr_consistency_validation() {
    ...
    // Test case 3: Check Dolby Vision report shows DV status only for DV content
    // Create a mock DV input MediaInfo object
    let mut dv_input_media = input_media.clone();
    let dv_stream = &mut dv_input_media.streams[0];
    
    // Add Dolby Vision tag to stream
    dv_stream.tags.insert("DOVI".to_string(), "1".to_string());
    
    // Check validation with DV input and non-DV output
    let mut report3 = ValidationReport::new();
    validate_hdr_consistency(&dv_input_media, &output_media_sdr, &mut report3).unwrap();
    
    // Should have a warning for DV to SDR conversion
    assert!(report3.warnings().len() > 0);
    
    // Check report contents
    let formatted3 = report3.format();
    assert!(formatted3.contains("Dolby Vision detection"));
    assert!(formatted3.contains("Dolby Vision to HDR10/SDR conversion detected"));
    
    // Test case 4: Non-DV input should not show DV warning
    // Use a normal SDR input and output
    ...
    validate_hdr_consistency(&sdr_input_media, &sdr_input_media.clone(), &mut report4).unwrap();
    
    // Should NOT have a warning for DV conversion with SDR input
    assert_eq!(report4.warnings().len(), 0);
    
    // Should not include Dolby Vision info in the report at all for non-DV content
    let formatted4 = report4.format();
    assert!(!formatted4.contains("Dolby Vision to HDR10/SDR conversion detected"));
}