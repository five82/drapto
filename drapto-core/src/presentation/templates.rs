...
pub fn render(template_data: TemplateData) {
    match template_data {
        ...
        TemplateData::CompletionSummary {
            title,
            success_message,
            groups,
        } => {
...
fn render_section_header(title: &str) {
    ...
    println!(
        "\n{}\n",
        style(format!("----- {} -----", title.to_uppercase()))
            .cyan()
            .bold()
    );
}
...
fn render_file_progress_header(current: usize, total: usize) {
    ...
    println!(
        "\n{}",
        style(format!("────▶ FILE {} OF {} ────", current, total))
            .magenta()
            .bold()
    );
}
...
mod tests {
    ...
    fn test_key_value_list_template() {
        let items = vec![("Input file", "test.mkv"), ("Duration", "00:02:00")];
