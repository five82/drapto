...
use crate::utils::{is_valid_video_file, SafePath};
...
pub fn find_processable_files(input_dir: &Path) -> CoreResult<Vec<PathBuf>> {
    ...
    if !input_dir.exists() {
        return Err(CoreError::PathError(
            format!("Directory does not exist: {}", input_dir.display())
        ));
    }
    ...
    let read_dir = std::fs::read_dir(input_dir)
        .map_err(|e| CoreError::PathError(
            format!("Cannot read directory {}: {}", input_dir.display(), e)
        ))?;
    ...
    if !errors.is_empty() {
        log::warn!("Errors during file discovery in {}: {}", 
                  input_dir.display(), errors.join(", "));
    }
    
    // Log summary
    if skipped_count > 0 {
        log::debug!("Skipped {} non-video files in {}", 
                   skipped_count, input_dir.display());
    }
    ...
    log::info!("Found {} video files in {}", files.len(), input_dir.display());
...
#[allow(dead_code)]
pub fn find_processable_files_recursive(input_dir: &Path, max_depth: usize) -> CoreResult<Vec<PathBuf>> {
    if max_depth == 0 {
    ...
    while let Some((current_dir, depth)) = directories_to_search.pop() {
        match find_processable_files(&current_dir) {
            ...
            Err(e) => {
                log::warn!("Failed to search directory {}: {}", current_dir.display(), e);
                continue;
            }
        }
    ...
    if all_files.is_empty() {
        ...
        log::info!("Found {} video files across directory tree starting from {}", 
                  all_files.len(), input_dir.display());
        Ok(all_files)
    }
}
