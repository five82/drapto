    ...
    fn handle(&self, event: &Event) {
        match event {
            ...
            Event::InitializationStarted {
                ...
                info!(
                    "Input: {} (duration: {}, resolution: {} {})",
                    input_file, duration, resolution, category
                );
                info!("Output: {}", output_file);
                info!(
                    "Dynamic range: {}, Audio: {}",
                    dynamic_range, audio_description
                );
            }
            ...
            Event::EncodingProgress {
                ...
                if *percent > 0.5 && *speed > 0.1 {
                    ...
                    let should_log = if let Some(last_log_time) = *last_time {
                        ...
                        let time_fallback =
                            now.duration_since(last_log_time) >= Duration::from_secs(300);

                        // Special: Major milestones (25%, 50%, 75%)
                        let major_milestone = [25, 50, 75].contains(&current_percent)
                            && current_percent > *last_logged;
            ...
            Event::BatchComplete {
                ...
                info!(
                    "Batch encoding complete: {} of {} files successful",
