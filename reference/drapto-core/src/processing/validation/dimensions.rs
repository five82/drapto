...
pub fn validate_dimensions(
    ...
    let (is_crop_correct, crop_message) = match (expected_dimensions, actual_dimensions) {
        ...
        (None, None) => (false, Some("Could not read video dimensions".to_string())),
    };
...
mod tests {
    ...
    fn test_dimensions_validation() {
        ...
        assert!(
            message
                .unwrap()
                .contains("Crop applied correctly (1856x1044)")
        );
        ...
        assert!(
            message
                .unwrap()
                .contains("Expected 1920x1080, found 1856x1044")
        );
        ...
        assert!(
            message
                .unwrap()
                .contains("No crop expected, dimensions: 1856x1044")
        );
    }
}
